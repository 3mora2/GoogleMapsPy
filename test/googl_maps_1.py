...
def scrap_text(text):
    ...
    # global last
    index = 0
    for index, place in enumerate(maps.search(text, sleep_time=4), 1):
        # q = db.session.query(GoogleMapsDB).filter_by(hex_ids=place.hex_ids).first()
        # if q:
        #     continue
        ...
        print(index, max_index,len(old), place)
...
keyword = ["متجر بقالة", "سوبرماركت", ]
fi = False
# fi_tt = "region_index=1 city_index=1 district_index=48 متجر بقالة في الرياض حي جرير"
with open("w", "r", encoding="utf-8") as f:
    fi_tt = f.read()
# fi_tt = "region_index=1/13 city_index=67/686 district_index=2/49"

l1 = len(json_collect)
search_i = 1
for region_index, region in enumerate(json_collect, 1):
    l2 = len(region["cities"])
    for city_index, city in enumerate(region["cities"], 1):
        if not city.get("districts"):
            for k in keyword:
                ke = f"{k} في {city["name_ar"]}"
                pp = f"{region_index=}/{l1} {city_index=}/{l2} {ke}"
                if not fi and (fi_tt in pp or fi_tt in ke):
                    fi = True
                print(pp)
                # print(search_i,ke)
                search_i += 1
...
s="متجر بقالة في جدة حي الشويضي"